...
use App\Imports\EpidbiomedImport;
...
class EpidbiomedScreen extends Screen
{
    /**
    ...
    {
        return [
            'files' => UploadedFile::where(['which_depo' => 'epidbiomed'])->paginate(10),
        ];
    }
    ...
    {
        $file = UploadedFile::where(['which_depo' => 'epidbiomed'])->where(['uploaded' => 0]);
        if ($file->exists()){
            foreach ($file->get() as $file){
                Excel::import(new EpidbiomedImport($file->file_id), storage_path($file->file_url));
                $file->uploaded = 1;
                $file->save();
            }
        }
    ...
    {
        EpidbiomedData::where(['uploaded_file_id' => $file->id])->delete();
        $file->delete();
    }
}
