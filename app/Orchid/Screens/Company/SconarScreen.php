...
class SconarScreen extends Screen
{
    /**
    ...
    {
        return [
            'files' => UploadedFile::where(['which_depo' => 'sonar'])->paginate(10),
        ];
    }
    ...
    public function commandBar(): iterable
    {
        return [
            ...
            ModalToggle::make('Add File')
                ...
                ->icon('plus'),
            Link::make('Download')
                ->href(env('APP_FILE_URL').'/depo/sonar')->icon('download'),
        ];
    }
    ...
    {
        $file = UploadedFile::where(['which_depo' => 'sonar'])->where(['uploaded' => 0]);
        if ($file->exists()){
            foreach ($file->get() as $file){
                Excel::import(new SonarImport($file->file_id), storage_path($file->file_url));
                $file->uploaded = 1;
                $file->save();
            }
        }
    ...
    {
        SonarData::where(['uploaded_file_id' => $file->id])->delete();
        $file->delete();
    }
}
