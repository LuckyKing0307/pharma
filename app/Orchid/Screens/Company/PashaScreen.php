...
class PashaScreen extends Screen
{
    /**
    ...
    {
        return [
            'files' => UploadedFile::where(['which_depo' => 'pasha'])->paginate(10),
        ];
    }
    ...
    public function commandBar(): iterable
    {
        return [
            ...
            ModalToggle::make('Add File')
                ...
                ->icon('plus'),
            Link::make('Download')
                ->href(env('APP_FILE_URL').'/depo/pasha')->icon('download'),
        ];
    }
    ...
    {
        $file = UploadedFile::where(['which_depo' => 'pasha'])->where(['uploaded' => 0]);
        if ($file->exists()){
...
//                $file->save();
    ...
    {
        PashaData::where(['uploaded_file_id' => $file->id])->delete();
        $file->delete();
    }
}
